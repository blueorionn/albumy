...
LICENSES = [
    ...
    ("CC BY-NC 4.0", "https://creativecommons.org/licenses/by-nc/4.0/", True,
...
GENRES = [
    "Ambient", "Chill", "Classical", "Drum & Bass", "Electronic", "Hip-Hop",
    "House", "Jazz", "Lo-Fi Hip-Hop", "Pop", "Rock", "Synthwave", "Techno", "Trap",
]

# name -> (bio, dob, genres)
ARTISTS = [
    ("Neon Drift", "Synthwave producer from Lisbon chasing 1985 through analog synths.",
     date(1991, 6, 3), ["Synthwave", "Electronic"]),
    ("Kaya Sound", "Lo-fi beatmaker; tape hiss is a feature, not a bug.", None,
     ["Lo-Fi Hip-Hop", "Chill"]),
    ("Static Bloom", "Ambient electronics built from field recordings and granular textures.",
     date(1988, 11, 17), ["Electronic", "Ambient"]),
    ("Miro Lake", "Pianist and composer blending minimalism with ambient noise.", None,
     ["Ambient", "Classical"]),
    ("Bitwave Collective", "A rotating crew of house and DnB producers.", None,
     ["House", "Drum & Bass"]),
    ("Luna Vale", "Dream-pop songwriter for slow evenings.", date(1996, 2, 28),
     ["Pop", "Chill"]),
    ("Otto Gray", "Jazz guitarist sampling dusty records.", None, ["Jazz", "Lo-Fi Hip-Hop"]),
    ("Vela Nine", "Techno engineer; hypnotic loops, relentless kick drums.", None,
     ["Techno", "Electronic"]),
    ("Rue Mirage", "Hip-hop producer with a taste for vintage soul samples.", None,
     ["Hip-Hop", "Trap"]),
    ("Juno Theory", "Indie rock four-piece that never met a chorus they didn't like.", None,
     ["Rock", "Pop"]),
]

# title, artist, genres, album_type, release_date, tracks
ALBUMS = [
    ("Midnight Circuit", "Neon Drift", ["Synthwave", "Electronic"], "album", date(2025, 6, 13),
     ["Chrome Heart", "Midnight Circuit", "Neon Rain", "Afterhour Drive", "Signal Lost", "Turbo Dusk"]),
    ("Paper Rooms", "Kaya Sound", ["Lo-Fi Hip-Hop", "Chill"], "ep", date(2026, 1, 30),
     ["Paper Rooms", "Sunday Static", "Cold Coffee", "Window Seat", "Paper Rooms (Instrumental)"]),
    ("Tidal Static", "Static Bloom", ["Electronic", "Ambient"], "album", date(2025, 11, 2),
     ["Undertow", "Tidal Static", "Floodplain", "Salt Air", "Driftglass", "Ebb", "Undertow (Instrumental)"]),
    ("Northbound", "Miro Lake", ["Ambient", "Classical"], "ep", date(2024, 9, 21),
     ["First Light", "Northbound", "Pine & Snow", "Harbor"]),
    ("Voltage Bloom", "Bitwave Collective", ["House", "Drum & Bass"], "album", date(2026, 3, 14),
     ["Voltage Bloom", "Circuitry", "Overclock", "Mainline", "Bright Machines", "Halfstep"]),
    ("Afterglow", "Luna Vale", ["Chill", "Pop"], "ep", date(2026, 5, 8),
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        track_count = 0
        for title, artist_name, album_genres, album_type, release_date, titles in ALBUMS:
            album = self._seed_album(title, artists[artist_name], album_type, release_date,
                                     [genres[g] for g in album_genres])
            for number, track_title in enumerate(titles, start=1):
                self._seed_track(
                    track_title, artists[artist_name],
                    [genres[g] for g in album_genres],
                    self._pick_license(licenses), album=album, track_number=number,
        ...
        for title, artist_name, track_genres, instrumental in SINGLES:
            self._seed_track(
                title, artists[artist_name], [genres[g] for g in track_genres],
                self._pick_license(licenses), instrumental=instrumental,
            )
            track_count += 1
    ...
    def _seed_licenses(self):
        ...
        for name, url, attribution, description in LICENSES:
            obj, _ = License.objects.get_or_create(
                name=name,
                defaults={"url": url, "requires_attribution": attribution, "description": description},
            )
            out[name] = obj
        return out
    ...
    def _seed_track(self, title, artist, genres, license_obj, album=None,
                    track_number=None, instrumental=False):
